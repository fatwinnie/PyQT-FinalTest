...
class CamShow(QMainWindow,Ui_CamShow):
    ...
    def drawAvg(self):
        ...
        for i in range(640):
            r_arr[i,0] = np.sum(img2RGB[260:280 , i , 0] )/ 20
            g_arr[i,0] = np.sum(img2RGB[260:280 , i , 1] )/ 20
            b_arr[i,0] = np.sum(img2RGB[260:280 , i , 2] )/ 20
            Gray_arr[i,0] = np.sum(gray[260:280 , i ])/ 20
    ...
    def drawAvg_after_calculate(self):
        ...
        self.save_arr = np.zeros((640,2),dtype=np.float16)
        for ii in range(640):
            self.save_arr[ii,0] = (x_640 - x_0) / 640 * (ii + 1)
            self.save_arr[ii,1] = Gray_arr[ii,0]

        self.draw_3.setRange(xRange=[0,x_640]) # 固定x軸 不會拉動
        self.draw_3.setRange(yRange=[np.amin(Gray_arr),np.amax(Gray_arr)]) # 固定y軸 不會拉動    

        self.draw_3.plot(_x,Gray_arr[:,0],pen='w')
